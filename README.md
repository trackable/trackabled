...
into the `trackabled` repository. They can be edited and committed just as if they
were normal files.
[git-subtree]: https://github.com/apenwarr/git-subtree

If you create a commit that contains files both from a subtree, and from the
`trackabled` source tree, please use care when designing the commit message, since
...
`trackable` in the same commit at all.
...
| `protobuf`      | https://github.com/google/protobuf           | Protocol buffer data interchange format. Trackable has changed some names in order to support the unity-style of build (a single .cpp added to the project, instead of linking to a separately built static library). |
| `trackable`     | N/A                                          | **Core source code for `trackabled`** https://github.com/trackable/trackabled-src|
...
| `test`          | N/A                                          | **Unit tests for `trackabled`** |
